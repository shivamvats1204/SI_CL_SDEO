...
- **Representative Sampling Across Datasets**: Samples placements across each generated dataset so reported metrics reflect the full workload more fairly than a first-block slice.
...
The execution is driven entirely by `main.py`. This script initializes the simulated DataNodes, generates HDFS-inspired workloads, samples representative placements across each dataset, executes both the SI-CL-SDEO and MBFOA algorithms under different stress loads, and produces comparative visual graphs.
...
- Runtime and reliability figures are simulated outputs from the heuristic model in this repository; they are not measurements from a live Hadoop cluster.
